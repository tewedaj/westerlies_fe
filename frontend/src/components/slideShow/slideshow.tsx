...
const Slideshow = ({ slides, navigator }: any) => {
  ...
  return (
    ...
    //  (index == currentIndex) &&
     <div key={index} className={`learn-more ${index === currentIndex ? '' : 'hide'}`}>
          ...
          <div className="learn-more-images">
            ...
            <img className="store-img" src={slide.profilePicture?.replace("http://","https://").replace("api.westerlies.io","apibeta.westerlies.com")} />
          </div>
          <img className="arrowbutton" src={arrowbutton} />
        </div>
    ))}
